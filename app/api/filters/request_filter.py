...
class DispatcherRequestFilter(DocumentFilter):
    ...
    __filters__ = {
        "status__in": Filter[RequestStatus](
            ...
            many=True,
            t_parser=StandartParser.get_enum_parser(RequestStatus),
            description="Статус заявки",
        ),
        "type": Filter[RequestType](
            q_func=lambda x: {"_type": x},
            t_parser=StandartParser.get_enum_parser(RequestType),
            description="Тип заявки",
        ),
        "source": Filter[RequestSource](
            q_func=lambda x: {"source": x},
            t_parser=StandartParser.get_enum_parser(RequestSource),
...
class DispatcherRequestReportFilter(DispatcherRequestFilter):
    """
    Класс с фильтрами для заявок жителя
    """
